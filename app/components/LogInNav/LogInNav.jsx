...
export default function LogInNav() {
  ...
  const createOrUpdateUserProfile = async (authUser) => {
    if (!authUser) {
      console.warn(
        "creatreOrUpdateUserProfile called with no authenticated user "
      );
      return;
    }
    ...
    try {
      console.log(
        "Attempting to create or update user profile for:",
        authUser.id
      );
      ...
      const { data, error } = await supabase.from("users").upsert(
        {
          ...
          emailVerified: authUser.email_verified_at || null,
        },
        {
